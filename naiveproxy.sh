...
inscaddynaive(){
naygvsion=`curl -s "https://raw.githubusercontent.com/fy-deng/naiveproxy/main/version"`
...
insupdate
cd /root
wget -N --no-check-certificate https://raw.githubusercontent.com/fy-deng/naiveproxy/main/caddy2-naive-linux-${cpu}.tar.gz
wget -qN --no-check-certificate https://raw.githubusercontent.com/fy-deng/naiveproxy/main/version
...
insweb(){
readp "设置naiveproxy伪装网址，注意：不要带https（不懂的话，直接回车跳过，默认为 www.nextcloud.com ）：" web
if [[ -z ${web} ]]; then
naweb=www.nextcloud.com
else
naweb=$web
fi
blue "已确认伪装网址：${naweb}\n"
...
red "======================================================================================"
url="naive+https://${user}:${pswd}@${ym}:$port?padding=true#Naive-1"
